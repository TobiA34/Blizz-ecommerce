...
class startup_shop_Body_Layout{
	/**
	...
	public function __construct() {
		...
		add_action('startup_shop_container_wrap_start', array( $this, 'container_wrap_start' ), 5 );
		add_action('startup_shop_container_wrap_start', array( $this, 'container_wrap_column_start' ), 10 );
		
		add_action('startup_shop_container_wrap_end', array( $this, 'container_wrap_column_end' ), 5 );
		add_action('startup_shop_container_wrap_end', array( $this, 'get_sidebar' ), 10 );
		add_action('startup_shop_container_wrap_end', array( $this, 'container_wrap_end' ), 999 );
	...
	*/
	function container_wrap_start(){
		...
		$html  = '<div id="primary" class="content-area container">
   		...
   		$html  = apply_filters( 'startup_shop_container_wrap_start_filter', $html );	
	...
	*/
	function container_wrap_column_start ( $layout = '' ){
	   ...
	   $html  	 = apply_filters( 'startup_shop_container_wrap_column_start_filter', $html );	
	...
	*/
	function container_wrap_column_end ( $layout = '' ){
	   ...
	   $html  	 = apply_filters( 'startup_shop_container_wrap_column_end_filter', $html );	
	...
	*/
	function container_wrap_end(){
		...
		$html  = '</div></div>';
   		...
   		$html  = apply_filters( 'startup_shop_container_wrap_end_filter', $html );	
...
$startup_shop_body_layout = new startup_shop_Body_Layout();