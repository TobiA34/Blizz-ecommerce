...
class startup_shop_Post_Related {
	/**
	...
	public function __construct() {
		...
		if( !is_admin()  )
		{
			add_action( 'startup_shop_site_content_type', array( $this,'site_loop_heading' ), 10 ); 
			add_action( 'startup_shop_site_content_type', array( $this,'site_content_type' ), 30 ); 
		}
		
		add_action( 'startup_shop_posts_blog_media', array( $this, 'render_thumbnail' ) ); 
		
		
		add_action( 'startup_shop_loop_navigation', array( $this,'site_loop_navigation' ) );
    ...
    function site_content_type( ){
		...
		$type = apply_filters( 'startup_shop_content_type_filter', startup_shop_get_option( 'blog_loop_content_type' ) );
	...
	*/
	public function content_read_more_link( $more  ) {
		...
		return sprintf( '<div class="more-link">
             <a href="%1$s" class="btn  theme-btn"><span>%2$s </span></a>
        </div>',
            esc_url( get_permalink( get_the_ID() ) ),
		    esc_html( startup_shop_get_option( 'read_more_text' ) )
        );
	...
	*/
	...
	function site_loop_navigation( $type = '' ) {
		...
		if( $type == '' ){
			$type = apply_filters( 'startup_shop_loop_navigation_filter', get_theme_mod( 'startup_shop_loop_navigation', 'default' ) );
		}
		...
		if( $type == 'default' ):
			...
			the_posts_navigation(
				array(
					'prev_text' => esc_html__('Previous Posts', 'startup-shop'),
					'next_text' => esc_html__('Next Posts', 'startup-shop'),
					'screen_reader_text' => __('Posts navigation', 'startup-shop')
				)
			);
			...
			the_posts_pagination( array(
				...
				'type' => 'list',
				'mid_size' => 2,
				'prev_text' => esc_html__( 'Previous', 'startup-shop' ),
				'next_text' => esc_html__( 'Next', 'startup-shop' ),
				'screen_reader_text' => esc_html__( '&nbsp;', 'startup-shop' ),
			) );
		echo '</div>';
		endif;
...
$startup_shop_post_related = new startup_shop_Post_Related();