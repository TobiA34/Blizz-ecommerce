...
class startup_shop_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'startup_shop_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'startup_shop_site_content_type', array( $this, 'render_meta_list' ), 20, 1 );
		add_action( 'startup_shop_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	public function render_meta_list( $order, $wrp_class = '' ) {
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= esc_html__( 'Posted by', 'startup-shop' );
				...
				case 'date':
					...
					$markup .= esc_html__( 'Posted on ', 'startup-shop' );
					$markup .= $this->get_time_tags();
					$markup .= '</li>';
					break;
				case 'category':
					$markup .= '<li class="meta category">';
					$markup .= esc_html__( 'Posted in ', 'startup-shop' );
					$markup .= get_the_category_list( ', ', get_the_ID() );
					$markup .= '</li>';
					break;
				case 'comments':
		...
		echo wp_kses( $markup, startup_shop_alowed_tags() );
	}
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'startup-shop' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'startup-shop' ) . ':</span>';
		foreach ( $tags as $tag ) {
...
$startup_shop_post_meta = new startup_shop_Post_Meta();